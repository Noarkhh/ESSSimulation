...
class Population:
    def __init__(self, size, fitness_change_factor, random_change_factor, number_of_behaviors=2,
        ...
        self.animals = [np.random.choice(number_of_behaviors, p=starting_animal_ratios) for _ in range(self.size)]
    ...
    def new_generation(self):
        ...
        # ADDING ANIMALS
        for random_animal_type in np.random.choice(self.behs, self.fitness_change,
                                                         p=self.last_gen_points / self.all_points):
            self.animals.append(random_animal_type)

        for _ in range(self.random_change):
            self.animals.append(np.random.randint(self.behs - 1))
...
population = Population(size=100000, fitness_change_factor=0.1, random_change_factor=0.001,
                        number_of_behaviors=3, starting_animal_ratios=(1/6, 2/6, 3/6), gens_in_sim=1000)
